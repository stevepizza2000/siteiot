...
function MainContent({
    ...
    useEffect(() => {
        ...
        const buscarDados = async () => {
            try {
                ...
                // Ordena as sessões da mais nova para a mais velha
                const sessoesOrdenadas = sessoesJson.sort(
                    (a, b) => new Date(b.inicioSessao) - new Date(a.inicioSessao)
                );

                // Encontra a sessão ativa (a primeira que não tem data de fim)
                const sessaoAtual = sessoesOrdenadas.find((sessao) => sessao.fimSessao === null) ?? null;

                // CORREÇÃO: Ordenar as temperaturas da mais antiga para a mais nova (cronológica)
                // Isso garante que o gráfico ande para frente e que o .at(-1) seja a temperatura REAL de agora
                const temperaturasSessaoAtual = (sessaoAtual?.temperaturas ?? []).sort(
                    (a, b) => new Date(a.registradoEm) - new Date(b.registradoEm)
                );

                const eventosSessaoAtual = (sessaoAtual?.eventos ?? []).sort(
                    (a, b) => new Date(a.registradoEm) - new Date(b.registradoEm)
                );
                ...
                setSessoes(sessoesOrdenadas); // Salva já ordenado
                ...
                if (!controller.signal.aborted) {
                    timerId = setTimeout(buscarDados, 500); // Mantido seu delay de 500ms
                }
            }
        };
    ...
    // Como já ordenamos lá em cima, o .at(-1) agora é 100% garantido de ser o último acontecimento
    ...
    // A sessão atual já foi definida no Fetch e guardada no State 'sessoes'
    const sessaoAtual = sessoes.find((sessao) => sessao.fimSessao === null) ?? null;
    
    // Fallback apenas para mostrar no bloco de "Registros" o que aconteceu na última, se o forno estiver desligado
    const ultimaSessao = sessaoAtual ?? sessoes[0] ?? null; 

    // CORREÇÃO: O forno SÓ está ligado se existir uma 'sessaoAtual' ATIVA.
    // Se 'sessaoAtual' for null, o forno está definitivamente desligado.
    const fornoLigado = sessaoAtual !== null && !String(sessaoAtual?.estadoFornoAtual ?? "").toUpperCase().includes("DESLIG");
                        ...
                        <ResponsiveContainer width="100%" height={300}>
                            <LineChart data={quentura}>
                                ...
                                <Line
                                    ...
                                    isAnimationActive={false} // Adicionado para evitar pulos estranhos na animação a cada 500ms
                                />
                            </LineChart>
                        </ResponsiveContainer>
                    ) : (
